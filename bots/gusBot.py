...
class GusBot(BotInterface):

    def __init__(self, name="GusBot"):
        super().__init__(name=name)

    def act(self, action_space: Sequence[Action], observation: Observation) -> Action:
        stage = observation.stage
        if stage == Stage.PREFLOP:
            return self.getNearestAction(self.handlePreFlop(observation, action_space), action_space)
        elif stage == Stage.FLOP:
            return self.getNearestAction(self.handleFlop(observation, action_space), action_space)
        elif stage == Stage.TURN:
            return self.getNearestAction(self.handleTurn(observation, action_space), action_space)
        elif stage == Stage.RIVER:
            return self.getNearestAction(self.handleRiver(observation, action_space), action_space)
    ...
    def handleFlop(self, observation: Observation, action_space: Sequence[Action]) -> Action:
        handPercent, cards = getHandPercent(
            observation.myHand, observation.boardCards)
        if handPercent <= .20:
            return Action.RAISE
        elif handPercent <= .60 or self.getFlushDraw(observation) or self.getStraightDraw(observation):
            return Action.CALL
        elif Action.CHECK in action_space:
            return Action.CHECK
        return Action.FOLD

    def handleTurn(self, observation: Observation, action_space: Sequence[Action]) -> Action:
        handPercent, cards = getHandPercent(
            observation.myHand, observation.boardCards)
        if handPercent <= .50:
            return Action.RAISE
